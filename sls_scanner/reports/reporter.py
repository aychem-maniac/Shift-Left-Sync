...
def export_html(target, vulns, port_info, path, timestamp):
    ...
    html_ports = (
        ...
        '<div class="tbl-wrap port-wrap"><table>'
    ...
    html_vuln_wrap = (
        ...
        '<button class="filter-btn risk-filter active" data-risk="ALL" onclick="filterRisk(\'ALL\')">전체</button>'
        '<button class="filter-btn risk-filter" data-risk="Critical" onclick="filterRisk(\'Critical\')">Critical</button>'
        '<button class="filter-btn risk-filter" data-risk="High" onclick="filterRisk(\'High\')">High</button>'
        '<button class="filter-btn risk-filter" data-risk="Medium" onclick="filterRisk(\'Medium\')">Medium</button>'
        '<button class="filter-btn risk-filter" data-risk="Low" onclick="filterRisk(\'Low\')">Low</button>'
        '<button class="filter-btn poc-filter" data-poc="CONFIRMED" onclick="filterPoc(\'CONFIRMED\')">확정</button>'
        '<button class="filter-btn poc-filter" data-poc="UNVERIFIED" onclick="filterPoc(\'UNVERIFIED\')">수동검토</button>'
        '</div>'
        '<div class="tbl-wrap vuln-wrap"><table id="vulnTable">'
        '<thead><tr>'
    ...
    js_block = (
        ...
        '  document.querySelectorAll(".risk-filter").forEach(function(b){'
        '    b.classList.remove("active");'
        '    if(b.getAttribute("data-risk")===r) b.classList.add("active");'
        '  });'
        '  renderTable();'
        '}'
        ...
        '  document.querySelectorAll(".poc-filter").forEach(function(b){'
        '    b.classList.remove("active");'
        '    if(currentFilter.poc && b.getAttribute("data-poc")===currentFilter.poc) b.classList.add("active");'
        '  });'
        '  renderTable();'
        '}'
